...
namespace Camera
{
	...
	bool Calibration::RunCalibrationAndSave(Calibration& s, cv::Size imageSize, cv::Mat&  cameraMatrix, cv::Mat& distCoeffs,std::vector<std::vector<cv::Point2f> > imagePoints )
	{
	...
	int Calibration::Start(int argc, char* argv[]){
		...
		cv::Size imageSize;
		int mode = s.inputType == Calibration::IMAGE_LIST ? CAPTURING : DETECTION;
		...
		for(int i = 0;;++i)
		{
			...
			if ( found)                // If done with success,
			{
				...
				if( s.calibrationPattern == Calibration::CHESSBOARD)
				{
					cv::Mat viewGray;
		...
		if( s.inputType == Calibration::IMAGE_LIST && s.showUndistorsed )
		{
